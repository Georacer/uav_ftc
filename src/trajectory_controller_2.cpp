 ...
 * 
 */
#include "trajectory_controller_2.hpp"
...
TrajectoryController::TrajectoryController(ros::NodeHandle n) : mpcController_(dt_, numConstraints_)
{
    ...
    Eigen::Matrix<real_t, NUM_STATES, 1> trimState = Eigen::Matrix<real_t, NUM_STATES, 1>::Zero();
    trimState(0) = referenceTrajectory_(0);
    trimState(1) = 0.034;
    mpcController_.setTrimState(trimState);

    // Set default reference inputs
    // refInputs_ << 0.0f, 0.0f, 0.0f, 0.5f, 0.0f;
    ...
    reference_ << referenceTrajectory_(0), referenceTrajectory_(1), defaultRefPsi,
                //  trimState(1), trimState(2), refInputs_;
                 trimState(1), trimState(2), refInputs_ , 0.0f; // with envcost
...
void TrajectoryController::step()
{
    ...
    // Manually entered
    const double  a0[3] = {      10.0,        1.0,        -19}; // Va top
    ...
    printf("%f,\t", a0[0]*states_(4) +  a0[1]*states_(0) +  a0[2]);
    printf("%f,\t", a1[0]*states_(4) +  a1[1]*states_(0) +  a1[2]);
    printf("%f,\t", a2[0]*states_(4) +  a2[1]*states_(0) +  a2[2]);
    printf("%f,\t", a3[0]*states_(4) +  a3[1]*states_(0) +  a3[2]);
    printf("Throttle cmd: %f\t", predictedControls_(3));
    // printf("Slack variable: %f\t", predictedControls_(4));
    // printf("Dummy variable: %f", states_(5));
    printf("\n");
...
void TrajectoryController::getReference(geometry_msgs::Vector3Stamped pRefTrajectory)
{
    ...
    // reference_.segment(NUM_STATES-1, refInputs_.size()) = refInputs_;
    reference_(9) = 0.0f; // envcost
